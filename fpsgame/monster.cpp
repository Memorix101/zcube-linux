...
namespace game
{
    ...
    {
        { GUN_BITE,      15, 100, 3, 0,   100, 800, 1,  10,  90, S_PAINO, S_DIE1,   "basiczombie", "ogro",       "ogro/vwep"},
         { GUN_BITE,      9, 120, 2, 0,   200, 400, 6,  14, 115, S_PAINE, S_DIE2, "nazizombie on a bike",     "monster/nazizombiebike",      NULL},
         { GUN_SG,        15, 120, 1, 0,  200, 400, 6,  14, 115, S_PAINE, S_DIE2, "nazizombie A",  "monster/nazizombie",        NULL},
         { GUN_BITE,      12, 120, 1, 0,  100,  400, 6,  14, 115, S_PAINE, S_DIE2, "nazizombie B", "monster/nazizombie2",        NULL},
        { GUN_RL,        12, 500, 1, 100, 200, 200, 6,  24, 210, S_PAINB, S_DEATHB, "zombieboss",  "monster/zombieboss", NULL },
        { GUN_BITE,      24,  50, 3, 0,   100, 400, 1,  15,  75, S_PAINP, S_PIGGR2, "fastzombie",  "monster/fastzombie",    NULL},
        { GUN_BITE,       4, 100, 1, 0,   200, 400, 2,  10,  60, S_PAIND, S_DEATHD, "female",    "monster/female",     NULL},
        { GUN_BITE,       4, 100, 1, 0,   200, 400, 2,  10,  60, S_PAINH, S_DEATHE, "zombie5",      "monster/zombie5",     NULL },
        { GUN_BITE,       3, 100, 1, 0,   200, 400, 1,  11,  50, S_PAINE, S_DEATHD, "zombie6",     "monster/zombie6",      NULL },
         { GUN_BITE,      5, 50,  1, 0,   200, 400, 1,  10,  30, S_PAINB, S_DEATHB, "zombie1",     "monster/zombie1",      NULL },
         { GUN_BITE,      3, 70,  1, 0,   200, 400, 1,  11,  50, S_PAIND, S_DEATHD, "zombie2",     "monster/zombie2",      NULL },
         { GUN_BITE,      3, 120, 1, 0,   200, 400, 1,  11, 150, S_PAINH, S_DEATHH, "zombie3",   "monster/zombie3",      NULL },
         { GUN_BITE,      2, 100, 1, 0,   200, 400, 1,  13, 120, S_PAINR, S_DEATHR, "zombie4",     "monster/zombie4",      NULL },
        { GUN_BITE,       2, 50,  1, 10000,  10000, 400, 1,  10,  90, S_PAIND, S_DEATHD, "zombie2 (innocent A)",    "monster/zombie2",      NULL },
        { GUN_BITE,       4, 50,  1, 10000,  10000, 400, 1,  10,  90, S_PAIND, S_DEATHD, "zombie2 (innocent B)",    "monster/zombie2",      NULL },
        { GUN_BITE,      13, 120, 1, 100, 100, 200, 1,  10,  160, S_PAINH, S_DEATHH, "fat zombie",    "monster/fatzombie",      NULL },
        { GUN_RIFLE,      4, 3500, 1, 100,  500, 0, 1, 17,  200, S_PAINH, S_DEATHH, "Kugelpanzer",    "monster/kugelpanzer",      NULL },
//        { GUN_RIFLE,      0, 100, 1, 0,  100, 0, 1, 17,  10000, S_PAINH, S_DEATHH, "Kugelpanzer",    "monster/nazisniper",      NULL },
        { GUN_BITE,       2, 500, 1, 0,   200, 400, 2,  10,  60, S_PAIND, S_DEATHD, "female",    "monster/female2",     NULL},
...
//        conoutf(CON_GAMEINFO, "\f2 Time %d s : %d zombies remaining", remain);
        if (remain==200) conoutf(CON_GAMEINFO, "\f2 %d zombies remaining", remain);
        if (remain==150) conoutf(CON_GAMEINFO, "\f2 %d zombies remaining", remain);
        if (remain==100) conoutf(CON_GAMEINFO, "\f2 %d zombies remaining", remain);
        if (remain==50) conoutf(CON_GAMEINFO, "\f2 %d zombies remaining", remain);
        if (remain>0 && remain<=10) conoutf(CON_GAMEINFO, "\f2 %d zombies remaining", remain);

        if (remain==200) playsound (S_V_BASECAP);
        if (remain==150) playsound (S_V_BASELOST);
        if (remain==100) playsound (S_V_FIGHT);
        if (remain==50) playsound (S_V_QUAD);
        if (remain==10) playsound (S_V_QUAD10);
...
//        conoutf(CON_GAMEINFO, "\f2--- single player time score: ---");
        conoutf(CON_GAMEINFO, "\f2 ");
        conoutf(CON_GAMEINFO, "\f2 ");
        int score = 0;
        conoutf(CON_GAMEINFO, "\f2Time taken: %d seconds", (lastmillis-maptime)/1000);
        conoutf(CON_GAMEINFO, "\f2Player deaths: %d", player1->deaths);
        conoutf(CON_GAMEINFO, "\f2Zombies destroyed: %d percent (n=%d)", (100*player1->frags)/((player1->frags+remain)), player1->frags+remain);
//        conoutf(CON_GAMEINFO, "\f2Zombies remaining: %d", remain);
        conoutf(CON_GAMEINFO, "\f2Accuracy: %d", accuracy);
//      conoutf(CON_GAMEINFO, "\f2Kills per second: %d", ((lastmillis-maptime)/player1->frags)/1000);

//        score = 1000 -(((lastmillis-maptime)/1000) + (player1->deaths*10) - (player1->frags));
        score = ((lastmillis-maptime)/1000) + (player1->deaths*30);
        ...
        defformatstring(nscore)("%d", bestscore);
        alias(aname, nscore);
//        conoutf(CON_GAMEINFO, "\f2TIMESCORE: %d  (best: %d)", score, bestscore);
        conoutf(CON_GAMEINFO, "\f2TIMESCORE: %d ", score);

//        conoutf(CON_GAMEINFO, "\f2TOTAL SCORE: %d seconds", score);
//        conoutf(CON_GAMEINFO, "\f2BEST SCORE: %d seconds", bestscore);
